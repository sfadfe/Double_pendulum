...
try:
    for epoch in pbar:
        ...
        if epoch % 20 == 0:
            ...
            val_loss_sum = 0.0
            with torch.no_grad():
                val_indices = torch.randperm(num_val_data, device=device)[:batch_size * val_check_batches]
                
                for i in range(0, len(val_indices), batch_size):
                    batch_idx = val_indices[i : i + batch_size]
                    
                    x_val_batch = X_val[batch_idx]
                    t_val_batch = T_val[batch_idx]
        ...
        if (epoch + 1) % 100 == 0:
            pbar.write(f"Epoch {epoch+1}/{epochs} | Val: {val_loss:.6f} | LR = {optimizer.param_groups[0]['lr']:.5e}")
